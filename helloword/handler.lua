...
local HelloWord = BasePlugin:extend()
local url = require "socket.url"
local http = require "resty.http"
local kong = kong
local data = kong.ctx.plugin

HelloWord.PRIORITY = "1000"
HelloWord.VERSION = "0.1.0"
-- 你的插件处理程序的构造函数。
-- 如果要扩展基本插件处理程序，它的唯一作用就是用名称实例化自己。
-- 该名称是您的插件名称，因为它将打印在日志中
function HelloWord:new()
    HelloWord.super.new(self, "request-management-plugin")
end

--实现请求分发
function HelloWord:access(config)
    -- Eventually, execute the parent implementation
    -- (will log that your plugin is entering this context)
    HelloWord.super.access(self)
    local bodyarray = {}
    local overlaidbody = {}
...
--@parameter api:发起子请求的地址
--@return 返回请求响应的body(json)
local function send_payload(api)
